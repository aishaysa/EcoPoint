...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Kelola Paket Penukaran</h1>
            <p class="text-sm text-slate-500">Daftar paket penukaran poin yang tersedia untuk user</p>
        </div>
        <a href="{{ route('admin.exchange-packages.create') }}" 
           class="inline-flex items-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg shadow-sm transition duration-200">
        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Jumlah Uang</th>
            ...
            <tbody class="bg-white divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/80 transition duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{{ $pkg->description }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold bg-blue-100 text-blue-800">
                            {{ number_format($pkg->points, 0, ',', '.') }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                        Rp {{ number_format($pkg->amount, 0, ',', '.') }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold {{ $pkg->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600' }}">
                            {{ $pkg->is_active ? 'Aktif' : 'Nonaktif' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        {{-- Edit --}}
                        <a href="{{ route('admin.exchange-packages.edit', $pkg) }}" 
                        ...
                        {{-- Form hapus --}}
                        <form action="{{ route('admin.exchange-packages.destroy', $pkg) }}" 
                              method="POST" 
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                        ...
                        <p class="text-lg font-medium">Belum ada paket penukaran</p>
                        <p class="text-sm">Klik tombol "Tambah Paket" untuk membuat paket pertama.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50 backdrop-blur-sm transition-all duration-300">
    <div id="deleteModalContent" class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all duration-300 scale-95 opacity-0 p-6">
        ...
        <h3 class="text-lg font-bold text-center text-slate-800 mb-2">Hapus Paket?</h3>
        <p class="text-sm text-center text-slate-600 mb-6">
            Apakah Anda yakin ingin menghapus paket <span id="modalNama" class="font-semibold text-slate-800"></span>? 
...
<script>
    ...
    (function() {
        ...
        cancelBtn.addEventListener('click', function() {
            ...
            hideModal();
            showCancelToast('Penghapusan paket "' + nama + '" dibatalkan');
        });
