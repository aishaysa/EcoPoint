        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $transaksis->firstItem() + $index }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $t->created_at->format('d M Y, H:i') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $t->pelanggan->nama ?? '-' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $t->jenisSampah->nama ?? '-' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ number_format($t->berat, 1) }}</td>
...
@php
    ...
    $status = $statusMap[$t->status ?? 'pending'] ?? ['bg' => 'bg-gray-100', 'text' => 'text-gray-800', 'label' => ucfirst($t->status ?? 'Unknown')];
@endphp
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold {{ $status['bg'] }} {{ $status['text'] }}">
                            {{ $status['label'] }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        {{-- Detail --}}
                        <a href="{{ route('admin.pelanggan.show', $t->id) }}" 
                ...
                <tr>
                    <td colspan="8" class="px-6 py-10 text-center text-gray-500">
                        ...
                        <p class="text-sm">Tambahkan transaksi baru untuk memulai.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
