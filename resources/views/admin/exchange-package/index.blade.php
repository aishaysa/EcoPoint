...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500">Daftar paket penukaran poin ke uang</p>
        </div>
    ...
    <div class="mb-4 flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-3 rounded-lg shadow-sm border border-gray-200">
        <div class="text-sm text-gray-500">
            Menampilkan <span class="font-semibold text-gray-800">{{ $packages->firstItem() ?? 0 }}</span> 
            - <span class="font-semibold text-gray-800">{{ $packages->lastItem() ?? 0 }}</span> 
            dari <span class="font-semibold text-gray-800">{{ $packages->total() }}</span> data
        </div>
        ...
        <nav class="flex items-center gap-1">
            ...
            @foreach ($packages->appends(request()->query())->links()->elements[0] ?? [] as $page => $url)
            ...
            @if (!count($packages->appends(request()->query())->links()->elements[0] ?? []))
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow-md border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($packages as $key => $pkg)
                <tr class="hover:bg-gray-50 transition duration-150">
                    {{-- Nomor urut nyambung antar halaman --}}
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $packages->firstItem() + $key }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ number_format($pkg->points) }}</td>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/50 backdrop-blur-sm transition-all duration-300">
    <div id="deleteModalContent" class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all duration-300 scale-95 opacity-0 p-6">
        ...
        <h3 class="text-lg font-bold text-center text-gray-800 mb-2">Hapus Paket Penukaran?</h3>
        ...
        toast.innerHTML = `
            <div class="bg-white rounded-2xl shadow-2xl border border-yellow-100 overflow-hidden relative">
                <div class="p-5 flex items-start gap-4">
                    ...
                    <button onclick="this.closest('#cancelToast').remove()" class="flex-shrink-0 mt-1 text-gray-400 hover:text-gray-600 transition-colors duration-200">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
